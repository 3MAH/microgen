...
class BoxMesh(PhaseMesh):
    ...
    def from_pyvista(
        ...
        try:
            ...
            return BoxMesh(pvmesh.points, elements, pvmesh, name)
    ...
    def construct(
        ...
        all_edges_corners = np.hstack((self.edge_list_XmYm, self.edge_list_XpYm, self.edge_list_XpYp,
                                       self.edge_list_XmYp, self.edge_list_XmZm, self.edge_list_XpZm,
                                       self.edge_list_XpZp, self.edge_list_XmZp, self.edge_list_YmZm,
                                       self.edge_list_YpZm, self.edge_list_YpZp, self.edge_list_YmZp,
                                       all_corners))
        ...
        self.face_list_Xm = self.face_list_Xm[
            np.lexsort((crd[self.face_list_Xm, 1], crd[self.face_list_Xm, 2].round(decimal_round)))]
        self.face_list_Xp = self.face_list_Xp[
            np.lexsort((crd[self.face_list_Xp, 1], crd[self.face_list_Xp, 2].round(decimal_round)))]
        self.face_list_Ym = self.face_list_Ym[
            np.lexsort((crd[self.face_list_Ym, 0], crd[self.face_list_Ym, 2].round(decimal_round)))]
        self.face_list_Yp = self.face_list_Yp[
            np.lexsort((crd[self.face_list_Yp, 0], crd[self.face_list_Yp, 2].round(decimal_round)))]
        self.face_list_Zm = self.face_list_Zm[
            np.lexsort((crd[self.face_list_Zm, 0], crd[self.face_list_Zm, 1].round(decimal_round)))]
        self.face_list_Zp = self.face_list_Zp[
            np.lexsort((crd[self.face_list_Zp, 0], crd[self.face_list_Zp, 1].round(decimal_round)))]

        self.corners = [self.corner_list_XmYmZm, self.corner_list_XmYpZm, self.corner_list_XpYmZm,
                        self.corner_list_XpYpZm,
                        self.corner_list_XmYmZp, self.corner_list_XmYpZp, self.corner_list_XpYmZp,
                        self.corner_list_XpYpZp]

        self.edges = [self.edge_list_XmYm, self.edge_list_XpYm, self.edge_list_XpYp, self.edge_list_XmYp,
                      self.edge_list_XmZm, self.edge_list_XpZm,
                      self.edge_list_XpZp, self.edge_list_XmZp, self.edge_list_YmZm, self.edge_list_YpZm,
                      self.edge_list_YpZp, self.edge_list_YmZp]

        self.faces = [self.face_list_Xm, self.face_list_Xp, self.face_list_Ym, self.face_list_Yp, self.face_list_Zm,
                      self.face_list_Zp]
    ...
    def _closest_points_on_faces(
        ...
        all_face_Xp = np.hstack((self.face_list_Xp, self.edge_list_XpYm, self.edge_list_XpYp, self.edge_list_XpZm,
                                 self.edge_list_XpZp, self.corner_list_XpYmZm, self.corner_list_XpYpZm,
                                 self.corner_list_XpYmZp, self.corner_list_XpYpZp))
        all_face_Yp = np.hstack((self.face_list_Yp, self.edge_list_YpZm, self.edge_list_YpZp, self.edge_list_XmYp,
                                 self.edge_list_XpYp, self.corner_list_XmYpZm, self.corner_list_XpYpZm,
                                 self.corner_list_XmYpZp, self.corner_list_XpYpZp))
        all_face_Zp = np.hstack((self.face_list_Zp, self.edge_list_YmZp, self.edge_list_YpZp, self.edge_list_XmZp,
                                 self.edge_list_XpZp, self.corner_list_XmYmZp, self.corner_list_XpYmZp,
                                 self.corner_list_XmYpZp, self.corner_list_XpYpZp))
    ...
    def closest_cells_on_boundaries(
        ...
        for i, face in enumerate(faces_m):
            ...
            if USE_MULTI_RAY:
                ...
                raytraceresult = (
                np.asarray(intersection_points), np.asarray(intersection_rays), np.asarray(intersection_cells))

            list_ray_trace.append(raytraceresult)

        return {
            'face_Xp': (
            list_cells_for_each_face[0], list_ray_trace[0][0], list_cells_for_each_face[0][list_ray_trace[0][2]]),
            'face_Yp': (
            list_cells_for_each_face[1], list_ray_trace[1][0], list_cells_for_each_face[1][list_ray_trace[1][2]]),
            'face_Zp': (
            list_cells_for_each_face[2], list_ray_trace[2][0], list_cells_for_each_face[2][list_ray_trace[2][2]])
        }
