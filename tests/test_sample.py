...
def test_operations():
    ...
    elem = microgen.shape.Box(center=(0.5, 0.5, 0.5),
                              dim_x=1, dim_y=1, dim_z=1)
    shape1 = elem.generate()
    phase1 = microgen.Phase(shape=shape1)

    elem = microgen.shape.Box(center=(0, 0, 0),
                              dim_x=0.5, dim_y=0.5, dim_z=0.5)
    ...
    microgen.repeatGeometry(phase1, rve, grid={"x": 2, "y": 2, "z": 2})
